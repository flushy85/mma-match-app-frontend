...
const Fighters = ({ selectFighter, isFavorite, active, setActive }) => {
  ...
  const handleChange = (e) => {
    setWeightClasses(weightClasses.map(w => w.weight !== e.target.id ? w : {...w, show: !w.show}))
  }

  const removeActive = (index) => {
    let newArr
    if(index === 0){
      newArr = active.slice(1)
    } else {
      newArr = active.slice(0, 1) 
    }
    setActive(newArr)
  }
    ...
    <div className="page-container">
      <h1 className="page-title">Find Your Fighter</h1>
        <Form className="fighters-form" inline>
          <FormControl type="text" onChange={({ target }) => setFilter(target.value)}placeholder="Filter by name" className="mr-sm-2" />
