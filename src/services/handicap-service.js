...
const getKoHandicap = (data) => {
  let koWinPercent = data.wins.knockouts /data.wins.total * 100
  let koLossPercent = data.losses.knockouts / data.losses.total * 100 
...
const getSubHandicap = (data) => {
  let subWinPercent = data.wins.submissions / data.wins.total * 100 
  let subLossPercent = data.losses.submissions / data.losses.total * 100 
...
const getTimeHandicap = (data) => {
  ...
  data.fights.forEach(fight => { 
    if(fight.result === "win"){
      ...
      const roundMins = ((parseInt(fight.round) - 1) * 5) + parseInt(lastRoundArr[0])
